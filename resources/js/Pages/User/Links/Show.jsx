...
export default function Show({link, offers, landings = []}) {
    const [selectedOffer, setSelectedOffer] = useState(link.offer);
    const [availableLandings, setAvailableLandings] = useState(link.offer.links);

    const { data, setData, post, processing, errors, reset } = useForm({
        name: link.name,
        offer_id: link.offer_id || null,
        landing_id: link.landing_id || null,
    ...
    const submit = (e) => {
        ...
        post(route('link.update', link.id), {
            onSuccess: () => {
