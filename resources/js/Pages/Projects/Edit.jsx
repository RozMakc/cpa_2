...
export default function Edit({ project, integrations = [] }) {
    const initialInvites = project.inviting_sources?.length ? project.inviting_sources : [{ group: '', channel: '' }];
    const [previewUrl, setPreviewUrl] = useState(project.image_path ? `/storage/${project.image_path}` : null);
    const { data, setData, post, processing, errors } = useForm({
        _method: 'put',
        name: project.name || '',
        integrations: project.integrations || [],
        parsing_sources: project.parsing_sources || '',
        inviting_sources: initialInvites,
        mailing_groups: project.mailing_groups || '',
        use_own_groups: project.use_own_groups || false,
    ...
    const handleImage = (file) => {
        ...
        setPreviewUrl(file ? URL.createObjectURL(file) : (project.image_path ? `/storage/${project.image_path}` : null));
    };
        ...
        <AuthenticatedLayout pageTitle="Редактирование проекта">
            ...
            <form onSubmit={submit} className="rounded-xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03]">
                <div className="flex flex-col justify-between gap-5 border-b border-gray-200 px-5 py-4 sm:flex-row sm:items-center dark:border-gray-800">
                    <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">Редактирование проекта</h3>
                                    ...
                                    <div key={index} className="grid grid-cols-[1fr_auto] gap-3 rounded-lg border border-gray-200 p-3 dark:border-gray-800">
                                        <div className="space-y-3">
                                            <div>
                                                ...
                                                <TextInput value={source.group || ''} onChange={(e) => setInvite(index, 'group', e.target.value)} />
                                            </div>
                                            <div>
                                                <InputLabel value="Канал" />
                                                <TextInput value={source.channel || ''} onChange={(e) => setInvite(index, 'channel', e.target.value)} />
                                            </div>
                                        </div>
